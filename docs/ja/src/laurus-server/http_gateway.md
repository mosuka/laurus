...
レスポンスの `integer` / `float` / `boolean` / `date_time` / `geo` / `geo3d` オプションには常に `multi_valued` が含まれます（例: `"location": {"geo": {"indexed": true, "stored": true, "multi_valued": true, "doc_values": true}}`、`"seen_at": {"date_time": {"indexed": true, "stored": true, "multi_valued": true, "doc_values": true}}`、`"flags": {"boolean": {"indexed": true, "stored": true, "multi_valued": true, "doc_values": true}}`）。同じキーは `POST /v1/index` および `POST /v1/schema/fields` でも受け付けます。
...
- 配列が混在型もしくは非数値要素を含む（例: `[1, "x"]` や `[true, 1]`）、または 2D と 3D の地理 object を混在させている（例: `[{"lat": ...}, {"x": ...}]`）
